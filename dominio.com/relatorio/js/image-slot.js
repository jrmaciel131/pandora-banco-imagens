...
(() => {
  ...
  // ── Persistência no navegador (IndexedDB) ───────────────────────────────
  // Funciona num servidor web comum (sem o host "omelette"): as imagens
  // sobrevivem a recarregar/fechar no MESMO navegador. O namespace
  // (window.__slotNS) separa as imagens por relatório (impressão digital do
  // Excel), pra um cliente não herdar as imagens de outro nas mesmas posições.
  const IDB_OK = (typeof indexedDB !== 'undefined');
  const IDB_DB = 'relatorio-img', IDB_STORE = 'kv';
  let _idbP = null;
  function idbOpen() {
  ...
  function idbPut(key, val) {
    return idbOpen().then((db) => db && new Promise((res) => {
      try { const tx = db.transaction(IDB_STORE, 'readwrite'); tx.objectStore(IDB_STORE).put(val, key); tx.oncomplete = () => res(true); tx.onerror = () => res(false); }
      catch (e) { res(false); }
    }));
  }
  const nsKey = () => 'slots:' + (window.__slotNS || 'default');
  function applyStored(map) {
    if (!map || typeof map !== 'object') return;
    const merged = Object.assign({}, map, slots); // mudanças em memória vencem a hidratação
  ...
  function load() {
    ...
    loadP = fetch(STATE_FILE)
      ...
      .catch(() => {})
      .then(() => (IDB_OK ? idbGet(nsKey()) : null))
      .then((stored) => { applyStored(stored); loaded = true; subs.forEach((fn) => fn()); });
    return loadP;
  }

  // Troca de relatório: zera os slots da sessão (pra um cliente não herdar as
  // imagens de outro nas mesmas posições) e re-hidrata as imagens salvas deste
  // relatório. Chamado pelo render ao subir um Excel novo.
  window.__slotsReload = function (ns) {
    window.__slotNS = ns || 'default';
    slots = {}; tombstones.clear();
    subs.forEach((fn) => fn()); // limpa a tela enquanto busca
  ...
  window.__imgPersist = {
    ...
    putKV: (k, v) => (IDB_OK ? idbPut('kv:' + (window.__slotNS || 'default') + ':' + k, v) : Promise.resolve(false)),
  };
  ...
  let saveDirty = false;
  function save() {
    if (IDB_OK) idbPut(nsKey(), slots); // persiste no navegador (servidor web)
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '    <div class="cap"></div>' +
        '    <div class="sub">ou <u>selecione um arquivo</u></div></div>' +
        ...
        '</div>' +
        '<div class="ctl"><button data-act="replace" title="Trocar imagem">Trocar</button>' +
        '  <button data-act="clear" title="Remover imagem">Remover</button></div>' +
    ...
    async _ingest(file) {
      ...
      if (!file || ACCEPT.indexOf(file.type) < 0) {
        this._setError('Use uma imagem PNG, JPEG, WebP ou AVIF.');
        return;
      }
      ...
      try {
        ...
        if (gen !== this._gen) return;
        this._setError('Não consegui ler essa imagem.');
    ...
    _render() {
      ...
      // Controls and reframe entry gate on this. Editable quando há omelette
      // (host de artifact) OU IndexedDB (servidor web comum) — assim o
      // reenquadrar/Trocar/Remover funcionam num http.server normal, não só
      // no host. A persistência do crop cai no IndexedDB via setSlot→save.
      const editable = !!(window.omelette && window.omelette.writeFile) || IDB_OK;
      ...
      this._cap.textContent = this.getAttribute('placeholder') || 'Solte uma imagem';
